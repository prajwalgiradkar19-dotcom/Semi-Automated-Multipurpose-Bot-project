...
static esp_err_t cmd_handler(httpd_req_t *req) {
 ...
 if (!strcmp(variable, "up")) {
  if (servo1Pos <= 170) {  // Increase only if servo1Pos is less than or equal to 170
    servo1Pos += 10;
  ...
  if (servo1Pos >= 10) {  // Decrease only if servo1Pos is greater than or equal to 10
    servo1Pos -= 10;
    servo1.write(servo1Pos);
  }
  Serial.println(servo1Pos);
  ...
  if (servo2Pos <= 170) {  // Increase only if servo2Pos is less than or equal to 170
    servo2Pos += 10;
  ...
  if (servo2Pos >= 10) {  // Decrease only if servo2Pos is greater than or equal to 10
    servo2Pos -= 10;
    servo2.write(servo2Pos);
  }
  Serial.println(servo2Pos);
...
// Trigger the camera and buzzer


void loop() {
  sendMessageToTelegram("Camera activated! View here: http://" + WiFi.localIP().toString());  // Pass the cameraLink to the function

  if (digitalRead(PIR_PIN) == HIGH) {
    digitalWrite(BUZZER_PIN, HIGH);  // Turn on buzzer
    String cameraLink = "Camera activated! View here: http://" + WiFi.localIP().toString();
    sendMessageToTelegram("Motion detected!"); // Send Telegram notification
    sendMessageToTelegram("Camera activated! View here: http://" + WiFi.localIP().toString());  // Pass the cameraLink to the function
